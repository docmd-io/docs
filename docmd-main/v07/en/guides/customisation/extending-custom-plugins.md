...
`docmd` features a reliable, hook-based [Plugin API](../../plugins/building-plugins.md). You can write simple Node.js modules that intercept the documentation lifecycle at various stages - from initial configuration to final HTML generation - allowing you to arbitrarily modify content and behaviour.
...
export default {
  ...
  // Intercepts page context before template rendering
  async onBeforeRender(page) {
    if (!page.html) return;
    // Replace placeholders with dynamic data in both HTML and frontmatter
