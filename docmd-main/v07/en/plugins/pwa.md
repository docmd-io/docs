...
The `@docmd/plugin-pwa` plugin transforms your documentation into a Progressive Web App (PWA). It adds a web manifest for mobile installation and registers a service worker for intelligent offline caching, ensuring your technical manuals remain accessible even in low-connectivity environments.

## Configuration

Customise your app branding within the `plugins` section of your `docmd.config.js`.
...
- **Offline Caching**: Uses a "Stale-While-Revalidate" strategy to serve pages instantly from the cache while updating in the background.
- **Mobile Installation**: Generates a `manifest.webmanifest` allowing users to "Add to Home Screen" on iOS and Android.
