...
*   **Intelligent Package Detection:** Handles `--no-save` proxies for bare repositories easily.
...
*   **Improved Failsafe Mechanics:** Expanded our internal CI/CD script (`failsafe.js`) to rigorously spawn dummy packages and assert all Installer regex behaviours on the fly so feature-regressions are impossible.
