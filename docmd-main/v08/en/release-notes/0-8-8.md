...
### Updates — graph viewer hardened and made opt-in

Two regressions surfaced in the initial OKF release, both fixed before 0.8.8 ships:

- **Graph viewer was rendering an empty canvas.** `graph.js` read `window.OKF_GRAPH`, which the build never set — so the fallback `{nodes:[], links:[]}` kicked in silently and the user saw a chrome with no nodes and no console errors. Fixed: `graph.js` now `fetch()`es `graph.json` from its own directory at runtime, with explicit loading and error states.
- **Graph files now sit in `graph/` subdirectory.** `graph.html` became `graph/index.html` so the viewer is reachable at `/okf/graph/` without a custom filename, and `graph.json`, `graph.js`, `graph.css` joined it in the same folder. The catalog link in `index.md` follows.

The graph viewer is now also **opt-in via `plugins.okf.graph: true`**. The OKF spec does not require a viewer, so a clean spec-compliant bundle ships without it by default. Existing configs that set `generateGraphViewer: true` keep working (deprecated alias honoured for one release with a TUI warning); migrating is a one-line rename to `graph: true`.
