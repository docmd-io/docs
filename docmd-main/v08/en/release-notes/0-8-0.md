...
- **Intelligent Fallback**: `index.md` is always prioritised, with `README.md` serving as a fallback if no index is present.
...
- **Persistent Dev Workers**: During `npx @docmd/core dev`, the worker pool remains persistent across file saves. This eliminates the 200-500ms thread-startup latency commonly associated with Node.js workers, resulting in near-instant incremental rebuilds.
...
- **Git Plugin Persistent Cache**: The Git plugin now runs in the new `onBeforeBuild` hook and utilises a reliable persistent disk cache (`.docmd/cache/git-history.json`). This completely eliminates redundant `git log` shell subprocesses on subsequent builds, dropping the build time of large repositories (800+ pages) from ~18 seconds down to just ~3.5 seconds.
...
## Migration Guide
Upgrade by running `npm install @docmd/core@latest`. Most sites will see immediate performance gains without any configuration changes.
