...
├── docmd.config.json           ← root multi-project config
└── package.json
```
...
| `src` | Directory containing this project's `docmd.config.json` and content. |
...
Each project directory has its own `docmd.config.json` with full independent configuration. Do **not** include `src` or `out` keys - the parent config provides those automatically.
...
5. **Root config is minimal** - only `projects` should be in the root `docmd.config.json`
...
- `docmd.config.json` with different title, URL, and plugins
