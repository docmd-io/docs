...
Enable the [LLMs Plugin](../../plugins/llms.md) in your `docmd.config.json`. This plugin automatically generates a `llms-full.txt` file after every build, which concatenates your entire documentation set into a single, high-context file that can be easily consumed by Large Language Models.
