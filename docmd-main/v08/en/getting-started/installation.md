...
<!-- SCREENSHOT: Terminal output showing a successful `npx @docmd/core init` run - the generated file tree with docmd.config.json, docs/index.md, and the 'Ready!' message. -->

::: callout tip "After local install"
Once `@docmd/core` is added to your project dependency, you can use `docmd` (or `npm docmd`, `yarn docmd`, `bun docmd`) instead of `npx @docmd/core` for all commands.
:::
