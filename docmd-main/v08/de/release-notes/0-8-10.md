...
### 🔌 MCP-Server erhält zwei neue Tools

Der `docmd mcp`-Server stellt nun sechs Tools bereit (vorher vier) und liefert einen funktionierenden `prompts/list`-Handler, damit MCP-Clients, die die deklarierte `prompts`-Capability respektieren, eine saubere leere Liste erhalten statt eines "Method not found"-Fehlers.

Zwei neue Tools:

- **`list_docs(subdir?)`** — Listet alle Markdown-Dateien im Projekt auf, optional beschränkt auf ein Unterverzeichnis (eine Locale, eine Version, ein Guide-Abschnitt). Gibt relative Pfade zurück, damit der Agent den Dokumentationsbaum navigieren kann, bevor er einzelne Dateien liest. Das `subdir`-Argument wird durch dieselbe `safePath`-Grenze geschützt wie `read_doc`.
- **`get_config()`** — Liefert die aufgelöste `docmd.config` als JSON-Objekt: Titel, Quell- und Ausgabeverzeichnisse, konfigurierte Locales, Versionen und die Liste der aktivierten Plugins. Sensible Werte (API-Schlüssel, Analytics-IDs) werden zu einer sicheren Zusammenfassung umgebaut — Rohe-Geheimnisse werden niemals zurückgegeben.

Zusammen mit den bestehenden `search_docs`, `read_doc`, `validate_docs` und `get_llms_context` kann ein Agent nun die Projektstruktur erkunden (`get_config` → `list_docs` → `read_doc`), ohne zuvor Pfade zu raten oder blinde `search_docs`-Aufrufe zu machen.

### 🌐 Koreanische (`ko`) UI-Übersetzung

Dank eines Community-Beitrags (PR #170, von @moduvoice) liefert die eingebaute UI nun eine koreanische Übersetzung. Die Locale wird zur Laufzeit genauso geladen wie jede andere Übersetzung — `ko.json` in das jeweilige Verzeichnis zu legen ist der gesamte Registrierungsschritt, keine Quellcode-Änderung erforderlich.

| Paket | Schlüssel | Datei |
| :--- | :---: | :--- |
| `@docmd/ui` (Kern-Theme) | 44 | `packages/ui/translations/ko.json` |
...
Jede `ko.json`-Datei wurde verifiziert: identische Schlüsselmenge wie das entsprechende `en.json`, alle `{platzhalter}`-Token erhalten. Eine koreanischsprachige mit docmd erstellte Doku rendert die eingebaute UI-Oberfläche (Sidebar, Suche, 404-Seite, Cookie-Hinweis, Sprach-/Versionsumschalter, Git-Verlaufs-Widget, Threads-Diskussions-Plugin) auf Koreanisch statt auf Englisch zurückzufallen.

Zum Deaktivieren für ein Projekt ist keine Konfigurationsänderung nötig — die Locale wird nur verwendet, wenn das Projekt explizit `i18n.locales` mit `{ id: "ko", label: "한국어" }` konfiguriert.
...
11. **Koreanische (ko) UI-Übersetzung**: Community-Beitrag (PR #170, @moduvoice) ergänzt `ko.json` für `@docmd/ui` (44 Schlüssel), `@docmd/plugin-search` (8), `@docmd/plugin-git` (11) und `@docmd/plugin-threads` (42). Alle Schlüsselmengen sind identisch mit `en.json`; dank des Laufzeit-Übersetzungs-Loaders sind keine Quellcode-Änderungen erforderlich.
