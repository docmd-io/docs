...
`packages/plugins/okf/tests/index.test.ts` (6 Tests) + `packages/plugins/okf/tests/brute.test.ts` (15 Tests) einschließlich 12 real-fs-Szenarien. **21/21 bestehen**. Neuer Test übt den Standard-Aktiviert-Vertrag (ein Projekt ohne `plugins.okf`-Eintrag erhält trotzdem ein generiertes Bundle).
...
**342 Assertions über 9 kategorisierte Dateien, alle grün:**
...
| `packages/plugins/okf` | 21 / 21 | Unit + 12 real-fs-Szenarien |
