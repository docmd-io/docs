...
### Aktualisierungen — Graph-Viewer gehärtet und auf Opt-in umgestellt

Zwei Regressionen traten in der ursprünglichen OKF-Veröffentlichung auf, beide vor dem 0.8.8-Release behoben:

- **Graph-Viewer zeigte eine leere Leinwand.** `graph.js` las `window.OKF_GRAPH`, das der Build nie gesetzt hat — also griff lautlos der Fallback `{nodes:[], links:[]}` und der Benutzer sah eine Chrome ohne Knoten und ohne Konsolenfehler. Behoben: `graph.js` ruft jetzt `graph.json` zur Laufzeit aus dem eigenen Verzeichnis per `fetch()` ab, mit expliziten Lade- und Fehlerzuständen.
- **Graph-Dateien liegen jetzt im `graph/`-Unterverzeichnis.** `graph.html` wurde zu `graph/index.html`, sodass der Viewer unter `/okf/graph/` ohne benutzerdefinierten Dateinamen erreichbar ist; `graph.json`, `graph.js`, `graph.css` liegen im selben Ordner. Der Katalog-Link in `index.md` folgt.

Der Graph-Viewer ist jetzt außerdem **per Opt-in über `plugins.okf.graph: true` verfügbar**. Die OKF-Spezifikation verlangt keinen Viewer, daher wird standardmäßig ein sauberes, spezifikationskonformes Bundle ohne ihn ausgeliefert. Bestehende Konfigurationen mit `generateGraphViewer: true` funktionieren weiterhin (veralteter Alias wird für ein Release berücksichtigt, mit TUI-Warnung); die Migration ist eine einzeilige Umbenennung zu `graph: true`.
