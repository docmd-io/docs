...
### 🔍 Semantische Suche: Auto-Erkennung + Erstbuild-Fix

Wenn `plugins.search.semantic: true` gesetzt ist, installiert die Build-Pipeline `docmd-search` und seine Peer-Abhängigkeiten (`@huggingface/transformers`, `onnxruntime-node`) beim ersten Lauf. Aber das HTML-Modal wurde VOR der Installation gerendert, sodass das `data-semantic="true"`-Attribut nie gesetzt wurde — der Browser fiel auf Stichwortsuche zurück, obwohl der semantische Index auf der Festplatte existierte.

**Drei Änderungen:**

1. **Laufzeit-Auto-Erkennung** (`client.ts`): Der Browser-Client prüft nun `.docmd-search/manifest.json` per HEAD-Request, unabhängig davon, was `data-semantic` im Modal sagt. Ein Request pro Seitenaufruf — kein Performance-Einfluss.

2. **Single Source of Truth** (`index.ts`): `onConfigResolved` berechnet ein `searchConfig`-Objekt und stempelt es auf `config._searchConfig`. `generateScripts` und `getAssets` lesen beide aus diesem Objekt — keine duplizierten Prüfungen mehr.

3. **Korrekte TUI-Reihenfolge**: Die "fehlende Abhängigkeiten"-Warnung erscheint erst NACHDEM `ensureDocmdSearch` die Installation tatsächlich fehlgeschlagen ist — nicht davor.

### Verifikation

- `pnpm --filter @docmd/parser build`: sauber
- `pnpm --filter @docmd/core build`: sauber
- `node tests/runner.js --skip-setup`: **543 bestanden, 0 fehlgeschlagen** (vorher 537; +6 aus M-5-Regressionstests)
