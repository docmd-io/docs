---
title: "docmd-search"
description: "Motor de búsqueda semántica offline para documentación。本地向量嵌入，浏览器就绪的索引。"
---

Motor de búsqueda semántica offline para documentación。向量嵌入在构建时使用 ONNX Runtime 在本地生成。Cliente del Navegador执行关键字匹配和整数向量余弦相似度计算——无需向云服务发送任何数据，也不向用户分发神经网络权重。
...
在构建阶段，嵌入向量由你的计算机上的 ONNX Runtime 计算。Cliente del Navegador接收预先计算好的整数向量，并在本地计算词条匹配和向量得分。
...
| 页面 | 说明 |
| :--- | :---------- |
| [Primeros Pasos](getting-started) | 安装、首次运行与模型选择 |
| [Configuración](configuration) | 全局、项目与命令行配置选项 |
| [Cómo Funciona](how-it-works) | 架构、切分、量化与混合打分 |
| [CLI 命令行参考](cli) | 命令行选项、Flag 与退出代码 |
| [API Programable](api) | 用于自定义构建脚本的 Node.js API 方法 |
| [Cliente del Navegador](browser-client) | Cliente del Navegador集成 API 与打分逻辑 |
...
3. Cliente del Navegador直接搜索这些预构建的索引文件。
