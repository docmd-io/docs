...
浏览器客户端是一个轻量级搜索运行时（<3KB gzipped），用于加载预构建的静态搜索索引文件，并使用关键字匹配和整数向量余弦相似度进行打分，无需 Web Worker、WASM 或神经网络模型权重。
...
```html
<input type="text" id="search-input" placeholder="正在加载搜索索引..." disabled />
