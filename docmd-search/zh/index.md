---
title: "docmd-search"
description: "面向文档的离线语义搜索引擎。本地向量嵌入，浏览器就绪的索引。"
---

面向文档的离线语义搜索引擎。向量嵌入在构建时使用 ONNX Runtime 在本地生成。浏览器客户端执行关键字匹配和整数向量余弦相似度计算——无需向云服务发送任何数据，也不向用户分发神经网络权重。
...
在构建阶段，嵌入向量由你的计算机上的 ONNX Runtime 计算。浏览器客户端接收预先计算好的整数向量，并在本地计算词条匹配和向量得分。
...
| 页面 | 说明 |
| :--- | :---------- |
| [快速开始](getting-started) | 安装、首次运行与模型选择 |
| [配置说明](configuration) | 全局、项目与命令行配置选项 |
| [工作原理](how-it-works) | 架构、切分、量化与混合打分 |
| [CLI 命令行参考](cli) | 命令行选项、Flag 与退出代码 |
| [程序化 API](api) | 用于自定义构建脚本的 Node.js API 方法 |
| [浏览器客户端](browser-client) | 浏览器客户端集成 API 与打分逻辑 |
