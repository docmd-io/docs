...
description: "Installation docmd-search，配置向量嵌入模型，并对文档目录进行索引。"
---
...
::: tabs
== tab "全局Installation" icon:globe
```bash
npm install -g docmd-search
```

Installation全局嵌入引擎依赖项：
...
全局嵌入依赖项 (`@huggingface/transformers` 和 `onnxruntime-node`) 必须已Installation在你的环境变量路径中。
...
- [Konfiguration](configuration)  -  分块、Glob 排除项和路径选项
- [CLI 命令行参考](cli)  -  命令行选项与 Flag
- [Programmierbare API](api)  -  Node.js API 方法
